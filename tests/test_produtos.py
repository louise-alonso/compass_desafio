...
class TestProdutosAPI:
    ...
    def test_4_cadastrar_produto_sem_token_retorna_401(self):
        ...
        assert "ausente" in response.json()["message"].lower() or \
               "inválido" in response.json()["message"].lower()
    ...
    def test_7_excluir_produto_como_admin_retorna_200(self, token_admin):
        ...
        assert response.status_code == 200
        assert "excluído" in body.get("message", "").lower() or \
               "excluido" in body.get("message", "").lower()